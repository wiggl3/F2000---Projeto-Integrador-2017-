...
public class MENU : MonoBehaviour {
	public Button BotaoJogar,BotaoOpcoes,BotaoSair,BotaoLoad, BotaoCreditos;
	[Space(20)]
	...
	void Start () {
		...
		BotaoLoad.onClick.AddListener(() => Load());
	...
	private void ChecarResolucoes(){
		...
		for(int y = 0; y < resolucoesSuportadas.Length; y++){
			Resolucoes.options.Add(new Dropdown.OptionData() { text =
	...
	private void Opcoes(bool ativarOP){
		...
		name.gameObject.SetActive (!ativarOP);
		//
	...
	{
		dt.load = true;
		SceneManager.LoadScene (dt.cenaSalva);
